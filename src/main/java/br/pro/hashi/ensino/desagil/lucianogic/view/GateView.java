...
public class GateView extends FixedPanel implements ItemListener {
	...
	public GateView(Gate gate) {
		...
		for(int i = 0; i < inSize; i++) {
			...
			inSwitches[i] = new Switch();

			gate.connect(inSwitches[i], i);
		}
		...
		if(outSize == 1) {
			...
			for (int i=0; i < outSize; i++) {
			add(outBoxes[i], 184, (i+1) * 55, 20, 20);
	...
	public void itemStateChanged(ItemEvent event) {
		...
		inSwitches[i].setOn(inBoxes[i].isSelected());
		
		for (int e=0; e < outBoxes.length; e++){
			outBoxes[e].setSelected(gate.read(e));
		}
	}
